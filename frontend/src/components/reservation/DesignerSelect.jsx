                    ...
                    {designers.map((designer) => (
                        <div key={designer.id} className="flex p-6 mb-6 w-full border-b border-gray-300">
                            <div className="flex flex-1 ml-20">
                                {/* 디자이너 이미지 */}
                                <img src={designer.image} className="w-28 h-28 rounded-full mr-6"/>
                                <div className="flex flex-col flex-1">
                                    ...
                                    <div className="flex items-center gap-2 mt-2">
                                        <div className="flex flex-col items-center mx-4" onClick={() => handleLikeClick(designer.id)}>
                                            {/* 좋아요 */}
                                            <Heart className={`w-6 h-6 ${likedDesigners.includes(designer.id) ? 'text-red-500 fill-current' : 'text-red-500'}`}/>
