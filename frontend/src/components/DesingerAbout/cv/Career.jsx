...
export default function Career({ isEditable, resumeData }) {
  ...
  useEffect(() => {
    const fetchCrId = async () => {
      try {
        ...
        // resumeData가 있으면 사용
        if (resumeData && resumeData.d_id) {
          console.log("Career - cr_id 설정:", resumeData.d_id);
  ...
  useEffect(() => {
    const fetchEmploymentHistory = async () => {
      try {
        console.log("Career - 경력 데이터 초기화, resumeData:", resumeData);
  ...
  const handleDeleteCareer = (cr_id) => {
    const updatedHistory = employmentHistory.filter(
      (entry) => entry.cr_id !== cr_id
    );
        ...
        <div className="border p-8 rounded-lg w-full max-w-4xl">
          ...
          <div className="flex items-center mb-4">
            ...
            <div className="flex items-center">
              ...
              />
              <label htmlFor="lessThanMonth" className="cursor-pointer">
                1개월 미만
              </label>
              <div className="pl-2" />
              <input
              ...
              />
              <label htmlFor="moreThanMonth" className="cursor-pointer">
                1개월 이상
              </label>
            </div>
          </div>
          ...
          {employmentPeriod === "1개월 이상" && (
            <div className="flex items-center mb-4">
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange("cr_join_date", e.target.value)
                }
                disabled={!isEditable}
              />
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange("cr_out_date", e.target.value)
                }
                disabled={!isEditable}
              />
            </div>
