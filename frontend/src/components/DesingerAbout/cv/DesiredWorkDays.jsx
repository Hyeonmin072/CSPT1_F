...
export default function DesiredWorkDays({ isEditable, resumeData }) {
  ...
  const [reId, setReId] = useState(""); // 이력서 ID 상태
  ...
  useEffect(() => {
    const fetchAvailableDays = async () => {
      try {
        ...
        // resumeData가 있으면 사용
        if (resumeData && resumeData.availableDays) {
          console.log(
            "DesiredWorkDays - availableDays 설정:",
            resumeData.availableDays
          );
          setAvailableDays(resumeData.availableDays);
        } else {
          console.log("DesiredWorkDays - 기본 요일 목록 사용");
          setAvailableDays(listday); // 기본 요일 목록 사용
        }

        // resumeData에서 선택된 요일과 결정 타입 가져오기
        if (resumeData) {
          if (resumeData.selectedDays) {
            console.log(
              "DesiredWorkDays - selectedDays 설정:",
              resumeData.selectedDays
            );
            setSelectedDays(resumeData.selectedDays);
          }
          if (resumeData.decisionType) {
            console.log(
              "DesiredWorkDays - decisionType 설정:",
              resumeData.decisionType
            );
            setDecisionType(resumeData.decisionType);
          }
          if (resumeData.d_id) {
            console.log("DesiredWorkDays - reId 설정:", resumeData.d_id);
      ...
      <div className="border p-8 rounded-lg w-full max-w-4xl">
            ...
            {availableDays.map((day) => (
              <button
                ...
                className={`px-4 py-2 rounded ${
                  selectedDays.some(
                    (selected) => selected.wt_day === day.wt_day
                  )
        ...
        {decisionType === "SELECT_DAYS" && (
          <div className="mt-4 p-3">
            <label className="w-32 text-gray-700 font-bold">선택된 요일</label>
