        ...
        (message) => {
            ...
            setSelectedChat((prev) => {
              ...
              const updatedMessages = prev.messages.map((msg) => {
                if (unreadMessageIds.includes(msg.messageId) && !msg.read) {
                  hasUpdate = true;
  ...
  useEffect(() => {
    return () => {
      if (socket && socket.connected && selectedChat) {
        console.log("채팅방 퇴장 (컴포넌트 언마운트):", selectedChat.chatRoomId);
        socket.publish({
          destination: `/publish/chat/leave/${selectedChat.chatRoomId}`,
          body: JSON.stringify({}),
        });
      }
    };
              ...
              <div className="flex items-center space-x-2 mt-1">
                ...
                {isMe && (
                  <div
                    ...
                    style={{ color: msg.read ? "#3b82f6" : "#999" }}
                  >
                    {msg.read ? "읽음 ✓" : "안읽음 🔴"}
                  </div>
                )}
